...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(_ sender: AnyObject) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request as URLRequest) {
            ...
            guard error == nil else {
                print("Error performing request \(error.debugDescription)")
                return
            }
            
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode ,
                statusCode >= 200 && statusCode <= 299 else {
                    print("Your request returned status code other than 2xx!")
                    return
            }
            
            guard let data = data else {
                print("No data was returned for the request!!")
                return
            }
            ...
            do {
                ...
                print("Error parsing response data: \(data)")
                return
            }
            
            guard let responseStatusCode = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int else {
                print("No status code in response in \(parsedResult)")
                return
            }
            
            if shouldFavorite && !(responseStatusCode == 12 || responseStatusCode == 1) {
                print("Unrecognized status code in \(parsedResult)")
                return
            } else if !shouldFavorite && responseStatusCode != 13 {
                print("Unrecognized status code in \(parsedResult)")
                return
            }
